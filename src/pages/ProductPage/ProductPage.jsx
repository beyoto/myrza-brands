...
import { CATEGORY_LABELS, CURRENCY } from '../../constants/catalog.js';
...
function ProductPage() {
  ...
  const whatsappUrl = `${WHATSAPP_URL}?text=${encodeURIComponent(
    `Здравствуйте! Интересует товар "${product.name}" (${product.price} ${CURRENCY}).
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: product.name,
          text: `Интересует товар "${product.name}" — ${product.price} ${CURRENCY}`,
          url: productUrl,
        });
      } catch (err) {
